		...
		int width, int prec, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], length) + write(1, &buffer[1], i - 1));
		}
		...
		int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')
		{
			...
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(i, &buffer[ind], length) + write(1, &buffer[3], i - 3));
		}
		else if (!(flags & F_MINUS) && padd == ' ')
		{
			buffer[--ind] = 'x';
			...
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[3], 1 - 3) + write(1, &buffer[ind], length));
		}
		else if (!(flags & F_MINUS) && padd == '0')
		{
			if (extra_c)
