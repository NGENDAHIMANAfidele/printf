...
int print_pointer(va_list tyoes, char buffer[]' int flags,
		int width, int precision, int size)
{
	...
	num_addrs = (unsigned long)addrs;
	 ...
	 if ((flags & F_ZERO) && !(flags & MINUS))
		 padd = '0';
	 if (flags & F_PLUS)
		 extra_c = '+', length++;
	 else if (flags & F-SPACE)
		 extra_c = ' ', length++;
	 ind++;
...
int print_non_printable(va_list types, buffer[], int flags,
		int width, int precision, int size)
{
	int i, offset;
	char *str;
